...
class QA(db.Model):
    ...
    room = db.relationship("Room", foreign_keys='QA.room_id',
                           backref=backref("qas", lazy='dynamic'))
    user = db.relationship("User", foreign_keys='QA.user_id',
                           backref=backref("qas", lazy='dynamic'))
    ...
    def __repr__(self):
        return u'<QA {} {}>'.format(self.room_id, self.user_id)
